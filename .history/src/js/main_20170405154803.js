$(document).ready(function () {
	...
	menuCategoryAside.on('mouseenter', function() {
		var target = $(this).data('menu');
		clearTimeout(menuFadeOut);
		menuFadeIn = setTimeout(function() {
			subCategories.removeClass('active');
			$('.sub-categories__list[data-menu="' + target + '"]').addClass('active');
		}, 300);
	});
	menuCategoryAside.on('mouseleave', function(e) {
		clearTimeout(menuFadeIn);
		var hoverSubCategories = e.relatedTarget.closest('.sub-categories');
		if (!hoverSubCategories) {
